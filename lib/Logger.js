	...
	[ "bold", "underline", "italic", "inverse", "grey", "yellow", "red", "green", "blue", "white", "cyan", "magenta" ].forEach( function( style ) {
		...
		if ( typeof String.prototype[ style ] !== "function" ){
	...
	module.exports = new Class( {
		...
		, createSignature: function( color, bold ){
			...
			result += " > ";
			if ( pos ){
				var mpath = pos[ 2 ].replace( project.root, "/" );
				if ( mpath.lastIndexOf( "node_modules" ) >= 0 ){
					mpath = "nm:" + mpath.substr( mpath.lastIndexOf( "node_modules" ) + 12 );
				}
				var tlen = 65 - ( pos[ 1 ].length + mpath.length + pos[ 3 ].length + pos[ 4 ].length ) + 1;
				if ( tlen < 1 ) tlen = 1;
				result += ( ( mpath + " " ).grey  + pos[ 3 ].grey + ( ":" + pos[ 4 ] ).grey + ", ".grey + pos[ 1 ].grey ) + new Array( tlen ).join( " " );
			}
			result += " >>> ".grey;
