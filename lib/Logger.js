		...
		, _dir: function(data, margin, name, first, knownObjects ) {
			...
			switch (typeof data) {
				case 'object':
					...
					// set && weak set
					else if (type.set(data) || type.weakSet(data)) {
						// convert to array
						this._dir(Array.prototype.slice.call(data), margin, fnname+' [Set]', first, knownObjects);
					}

					// map && weak map
					else if (type.map(data) || type.weakMap(data)) {
						// get an object
						var currObj = {};
						var pair;
						var idx;
						...
						this._dir(currObj, margin, fnname+' [Map]', first, knownObjects);
					}
